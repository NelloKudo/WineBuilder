...
package_wine() {
    ...
    mv "${BUILD_OUT_TMP_DIR}" "$WINE_BUILD_NAME"

    if [ "${BUILD_FONTS}" = "true" ]; then
        # Launch fonts build script
        Info "Compiling and installing fonts from Proton..."
        cd "${WINE_ROOT}/protonfonts"
        WINE_FONTS_DESTDIR="${BUILD_DIR}/${WINE_BUILD_NAME}/share/wine/fonts" make all-dist
        cd "${BUILD_DIR}"
    fi

    local ARCHIVE_NAME="$WINE_BUILD_NAME${EXTRA_NAME:-}-${RELEASE_VERSION}-x86_64.tar.xz"

    Info "Creating and compressing ${ARCHIVE_NAME}..."
    tar -cJf \
        "${ARCHIVE_NAME}" \
        --xattrs --numeric-owner --owner=0 --group=0 "$WINE_BUILD_NAME"
...
main() {
    ...
    # Edit this to change the Wine build name
    WINE_BUILD_NAME="wine-staging-$WINE_VERSION"
